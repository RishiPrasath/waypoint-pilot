...
def test_missing_order_uses_problem_detail(csa_headers: dict[str, str]) -> None:
...
def test_invalid_order_id_uses_invalid_request_problem_detail(csa_headers: dict[str, str]) -> None:
    client = TestClient(app)

    response = client.get("/api/v1/orders/INVALID/status", headers=csa_headers)

    assert response.status_code == 400
    assert_problem_detail(response.json(), "INVALID_REQUEST", 400)


def test_deprecated_transition_code_is_not_returned(csa_headers: dict[str, str]) -> None:
    client = TestClient(app)
