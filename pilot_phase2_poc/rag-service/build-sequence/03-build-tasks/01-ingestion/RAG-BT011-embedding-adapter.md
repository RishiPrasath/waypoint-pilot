...
|---|---|
| Task ID | `RAG-BT011` |
| Task Name | Add Embedding Adapter |
| Build Stage | 01-ingestion - Ingestion |
| Source Question | RAG-Q004 |
| Decision / ADR | RAG-DT010, RAG-DT013 |
...
| Status | Planned |
| Evidence | `pilot_phase2_poc/rag-service/build-evidence/RAG-BT011-embedding-adapter.md` |

## 1. Task Definition

Build: embedding adapter interface and deterministic test adapter.

Goal: allow embedding model swapping while keeping tests lightweight and deterministic.

Module: `app/stages/stage_01_ingestion/embeddings/`.
...
- deterministic fixture adapter returns predictable vectors
- embedding dimension is explicit
- model/version metadata can be recorded
- real local model remains configurable and swappable
...
```text
pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/tests/test_embedding_adapter.py
```

### Windows PowerShell Test File Creation

```powershell
$TestPath = Join-Path $WorktreePath "pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/tests/test_embedding_adapter.py"
New-Item -ItemType Directory -Force -Path (Split-Path $TestPath) | Out-Null
@(
  '# RAG-BT011 failing test placeholder',
...
```bash
TEST_PATH="$WORKTREE_PATH/pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/tests/test_embedding_adapter.py"
...
```text
The test or acceptance check fails because embedding adapter interface and deterministic test adapter is not implemented yet.
```
...
- `pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/embeddings/base.py`
- `pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/embeddings/deterministic.py`

### Windows PowerShell Implementation File Preparation

```powershell
$PrimaryImplPath = Join-Path $WorktreePath "pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/embeddings/base.py"
New-Item -ItemType Directory -Force -Path (Split-Path $PrimaryImplPath) | Out-Null
# Create or update the implementation artifacts for RAG-BT011:
# pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/embeddings/base.py; pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/embeddings/deterministic.py
```

### Linux / macOS Bash Implementation File Preparation

```bash
PRIMARY_IMPL_PATH="$WORKTREE_PATH/pilot_phase2_poc/rag-service/app/stages/stage_01_ingestion/embeddings/base.py"
...
git -C $WorktreePath commit -m "build(rag): implement rag-bt011 embedding-adapter"
...
git -C "$WORKTREE_PATH" commit -m "build(rag): implement rag-bt011 embedding-adapter"
