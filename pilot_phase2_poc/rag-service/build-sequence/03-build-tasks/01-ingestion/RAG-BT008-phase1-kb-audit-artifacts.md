# RAG-BT008: Add Phase 1 KB Audit Artifacts
...
| Worktree Path | `C:\Users\prasa\Documents\Github\waypoint-pilot-worktrees\rag-bt008-phase1-kb-audit-artifacts` |
...
Build: Phase 1 KB source audit artifacts.

Goal: classify legacy Phase 1 KB material before any Phase 2 promotion.

Module: `docs/design/ and knowledge_base/registry/`.
...
- audit file exists
- audit table has required columns
- sample rows classify at least one keep, reference, archive, and drop source
- initial registry rows align with source registry schema
- legacy/phase1-kb-snapshot is used as audit input only

Out Of Scope:

- final canonical KB migration
- runtime ingestion of legacy files
...
$TaskId = "rag-bt008"
$Slug = "phase1-kb-audit-artifacts"
...
```text
pilot_phase2_poc/rag-service/tests/design/test_phase1_kb_audit_artifacts.py
```

### Windows PowerShell Test File Creation

```powershell
$TestPath = Join-Path $WorktreePath "pilot_phase2_poc/rag-service/tests/design/test_phase1_kb_audit_artifacts.py"
New-Item -ItemType Directory -Force -Path (Split-Path $TestPath) | Out-Null
@(
  '# RAG-BT008 failing test placeholder',
  '# Replace this placeholder with the task-specific failing test after design gates are complete.',
  'def test_phase1_kb_audit_artifacts():',
  '    assert False, "Implement RAG-BT008 after design dependencies are confirmed"'
...
```bash
TEST_PATH="$WORKTREE_PATH/pilot_phase2_poc/rag-service/tests/design/test_phase1_kb_audit_artifacts.py"
...
```text
The test or acceptance check fails because Phase 1 KB source audit artifacts is not implemented yet.
```
...
- `pilot_phase2_poc/rag-service/knowledge_base/registry/source_registry.sample.json`

### Windows PowerShell Implementation File Preparation

```powershell
$PrimaryImplPath = Join-Path $WorktreePath "pilot_phase2_poc/rag-service/docs/design/phase1-kb-source-audit.md"
New-Item -ItemType Directory -Force -Path (Split-Path $PrimaryImplPath) | Out-Null
# Create or update the implementation artifacts for RAG-BT008:
# pilot_phase2_poc/rag-service/docs/design/phase1-kb-source-audit.md; pilot_phase2_poc/rag-service/knowledge_base/registry/source_registry.sample.json
```

### Linux / macOS Bash Implementation File Preparation

```bash
PRIMARY_IMPL_PATH="$WORKTREE_PATH/pilot_phase2_poc/rag-service/docs/design/phase1-kb-source-audit.md"
...
uv run pytest "tests/design/test_phase1_kb_audit_artifacts.py" -q
uv run pytest -q
```
...
git -C $WorktreePath commit -m "build(rag): implement rag-bt008 phase1-kb-audit-artifacts"
...
git -C "$WORKTREE_PATH" commit -m "build(rag): implement rag-bt008 phase1-kb-audit-artifacts"
